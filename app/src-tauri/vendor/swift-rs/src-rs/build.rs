...
impl SwiftLinker {
    ...
    pub fn link(self) {
        ...
        for package in self.packages {
            ...
            // SoloPDF patch: Xcode 27's default `swiftbuild` backend appends its own
            // `-sdk MacOSX… -target …-macos` after our -Xswiftc flags, so iOS builds
            // compile against the macOS SDK and fail. The native backend honours them.
            if swift_build_supports_build_system() {
...
fn swift_build_supports_build_system() -> bool {
    Command::new("swift")
        .args(["build", "--help"])
        .output()
        .map(|o| String::from_utf8_lossy(&o.stdout).contains("--build-system"))
        .unwrap_or(false)
}
